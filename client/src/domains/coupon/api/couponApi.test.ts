...
describe('couponApi', () => {
  test('getCoupons는 preorderId로 사용 가능한 쿠폰 목록을 요청한다', async () => {
    ...
    let requestedIsRemoteArea: string | null = null;

    mockServer.use(
      http.get(`${API_BASE_URL}/coupons`, ({request}) => {
        const url = new URL(request.url);
        requestedPreorderId = url.searchParams.get('preorderId');
        requestedIsRemoteArea = url.searchParams.get('isRemoteArea');
    ...
    await expect(getCoupons('preorder-1', true)).resolves.toEqual({
      coupons: [
        {
          couponId: 1,
          code: 'FIXED5000',
    ...
    expect(requestedIsRemoteArea).toBe('true');
  });
  ...
  test('getCoupons 실패 시 쿠폰 기본 에러 메시지를 전달한다', async () => {
    ...
    await expect(getCoupons('preorder-1', false)).rejects.toThrow('쿠폰 요청에 실패했습니다.');
  });
});
