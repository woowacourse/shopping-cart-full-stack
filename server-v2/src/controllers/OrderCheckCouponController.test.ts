...
describe('OrderCheck coupon endpoints', () => {
    ...
    it('무료배송 쿠폰이 적용되면 도서산간 여부와 상관없이 배송비는 0원이다.', async () => {
        await request(app).patch('/carts/products/1').send({ quantity: 10 }).expect(200);

        await request(app).post('/order-check').expect(201);

        await request(app).patch('/order-check/select/remote-areas').send({ checkStatus: true }).expect(200);
