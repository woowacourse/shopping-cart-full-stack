...
export const patchCartItemQuantity = async (req: Request<{ productId: string }>, res: Response, next: NextFunction) => {
    try {
        const { productId } = req.params;
        const cartItem = await cartItemsService.updateCartItemQuantity(DUMMY_USER_ID, productId, req.body.quantity);
...
export const deleteCartItem = async (req: Request<{ productId: string }>, res: Response, next: NextFunction) => {
    try {
        const { productId } = req.params;
