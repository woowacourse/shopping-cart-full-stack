...
const validateSelectedCouponIdsShape = (selectedCouponIds: unknown): string[] => {
  ...
  if (
    !Array.isArray(selectedCouponIds) ||
    !selectedCouponIds.every((couponId) => typeof couponId === 'string')
  ) {
    throw new BadRequestError({
...
const getCouponApplicationOrder = (coupons: CouponRepository.CouponRecord[]) => {
  return [...coupons].sort(
    (left, right) => getCouponApplicationPriority(left) - getCouponApplicationPriority(right),
  );
};
...
const getBogoCouponDiscountAmount = (
  ...
  const eligibleProducts = order.products.filter(
    (product) => product.quantity >= minimumRequiredQuantity,
  );
    ...
    switch (coupon.discountType) {
      ...
      case 'BOGO': {
        const couponDiscount = Math.min(
          ...
          getBogoCouponDiscountAmount(order, coupon),
        );
        remainingOrderAmount -= couponDiscount;
        orderDiscountAmount += couponDiscount;
        break;
...
const isValidateBogoType = async (
  userId: string,
  ...
  return (
    orderCheckInfo?.products.some((product) => product.quantity >= minimumRequiredQuantity) ?? false
  );
};
...
export const getBestDiscountCouponCombination = async (
  userId: string,
) => {
  const order = await findOrderOrThrow(userId);
  const coupons = await CouponRepository.getAll();
  ...
  return buildCouponSelections(validCoupons).reduce<CouponSelectionResult>((best, selection) => {
    const candidate = {
...
export const getSelectedCouponDiscountSummary = async (
  userId: string,
) => {
  const selectedCouponIds = await CouponRepository.getSelectedCouponIds(userId);
