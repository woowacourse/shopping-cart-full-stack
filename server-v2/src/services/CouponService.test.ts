...
import {
  getBestDiscountCouponCombination,
  getDisCountAmount,
  selectCoupon,
...
describe('CouponService', () => {
  ...
  it('정액 쿠폰을 적용한 뒤 정률 쿠폰을 적용한다.', async () => {
    ...
    await expect(
      getDisCountAmount(['FIXED5000', 'MIRACLESALE'], 'coupon-user-fixed-and-percentage'),
    ).resolves.toBe(39500);
  });
  ...
  it('무료 배송 쿠폰은 기본 배송비와 도서산간 추가 배송비를 함께 할인한다.', async () => {
    ...
    await expect(
      getDisCountAmount(['FREESHIPPING', 'MIRACLESALE'], 'coupon-user-free-shipping'),
    ).resolves.toBe(27000);
  });
