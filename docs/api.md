...
2+1 쿠폰이 적용되면 서버는 무료 증정 상품의 정가를 `price.orderAmount`에 포함하고, 같은 금액을 `price.productDiscountAmount`에 포함한다. 따라서 화면의 주문 금액은 구매 수량과 무료 증정 수량의 정가 합계이며, 최종 결제 금액은 사용자가 실제 지불해야 하는 구매 수량 기준 금액이다.
