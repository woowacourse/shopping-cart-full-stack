...
describe('carts service 테스트', () => {
    ...
    describe('updateCartProduct 테스트', () => {
        it('필수 필드가 모두 존재하고 도메인 규칙에 맞는 경우 장바구니 상품 수량을 변경한다.', () => {
            ...
            const updatedProduct = cartsService.updateCartProduct(cartId, productId, cartUpdateOption);
        ...
        it('quantity가 누락된 경우 ServiceError를 던진다.', () => {
            const cartId = 1;
            const productId = 1;
            const cartUpdateOption = {} as Parameters<typeof cartsService.updateCartProduct>[2];
    ...
    describe('deleteCartProduct 테스트', () => {
        it('cartId와 productId에 해당하는 장바구니 상품을 제거한다.', () => {
            ...
            expect(cartAfterDelete.products).toHaveLength(cartBeforeDelete.products.length - 1);
