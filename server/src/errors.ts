...
export function handleErrors(res: express.Response, err: Error) {
  ...
  new InternalServerError().handle(res);
}
...
export class APIError extends Error {
  ...
  protected toResponse(): object {
    return { code: this.code, message: this.message };
  }

  public handle(res: express.Response): void {
    res.status(this.statusCode).send(this.toResponse());
  }
}
...
export class BadRequestError extends APIError {
  private readonly errors: Record<string, ErrorResponse>;
  constructor({
    code = "BAD_REQUEST",
